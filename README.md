...
- **EURC** is a standard ERC-20 at `0x89B5...D72a` with 18 decimals
- **Gas** is paid in USDC (native token)
- **CORS**: Arc RPC blocks browser requests from Vercel. Solved via `vercel.json` proxy (`/api/rpc` → `rpc.testnet.arc.network`)
